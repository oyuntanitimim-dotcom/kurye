...
<p class="text-sm text-slate-600 mb-6">Seçilen takvim ayına göre teslim bazlı mutabakat. Günlük satırlar teslim gününe (<code class="bg-slate-100 px-1 rounded text-xs">updated_at</code> tarihi) göre gruplanır.</p>

<form method="get" action="{{ route('firm.finance.reconciliation') }}" class="mb-6 flex flex-wrap gap-3 items-end text-sm">
    <div>
    ...
    <a href="{{ route('firm.finance.reconciliation', ['month' => $month, 'export' => 'csv']) }}" class="rounded-lg border border-slate-300 bg-white px-4 py-2 text-slate-800 hover:bg-slate-50">CSV indir</a>
</form>
