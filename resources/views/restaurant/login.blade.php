...
<div class="min-h-screen flex items-center justify-center p-6">
    <div class="w-full max-w-sm rounded-2xl border border-slate-200 bg-white p-8 shadow-sm">
        <h1 class="text-xl font-semibold mb-2">Firma girişi</h1>
        <p class="text-xs text-slate-500 mb-6">restoran@demo.local / password</p>
        <form method="post" action="{{ route('restaurant.login.store') }}" class="space-y-4">
            @csrf
