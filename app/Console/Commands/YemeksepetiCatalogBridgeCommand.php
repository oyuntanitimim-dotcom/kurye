...
class YemeksepetiCatalogBridgeCommand extends Command
{
    protected $signature = 'kurye:yemeksepeti-catalog-bridge
    ...
    protected $description = 'Yemeksepeti Catalog async job bridge (start + status).';
    ...
    public function handle(): int
    {
        ...
        try {
            ...
            if ($jobId !== '') {
                ...
                $res = $client->getCatalogJobStatus($chainId, $jobId);
            ...
            if ($chainId === '' || $vendorId === '') {
                $this->error('chain_id ve vendor_id gerekli.');
                return self::FAILURE;
            }

            // Dokümanda PUT /catalog => body {"products":[...]} şeklinde.
            if (! array_key_exists('products', $payload) || ! is_array($payload['products'])) {
                $this->error('Catalog payload icinde "products" array’i bekleniyor.');
                return self::FAILURE;
            }

            $res = $client->updateCatalogProducts($chainId, $vendorId, $payload);
            $this->info('Catalog async job tetiklendi.');
            $this->line(json_encode($res, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE));
            return self::SUCCESS;
        } catch (\Throwable $e) {
            $this->error('Catalog bridge hatasi: '.$e->getMessage());
            return self::FAILURE;
        }
    }
